...
import { getContentfulConnectionsByTeam, getUser, getUserWithTeam } from '@/lib/db/queries';

export async function GET(req: NextRequest, props: { params: Promise<{ spaceId: string; articleId: string }> }) {
  ...
  const connection = connections.find(conn => conn.spaceId === params.spaceId);
...
export async function PUT(req: NextRequest, props: { params: Promise<{ spaceId: string; articleId: string }> }) {
  const params = await props.params;
  const user = await getUser();
